...
CB_Node::CB_Node(CB_Node* _parent) : ID(CB_Node_ID++), parent(_parent) {
  thetaImprovement = rai::getParameter<double>("CB/thetaImprovement", .1);
}

void CB_Node::write(std::ostream& os) const {
  os <<'#' <<ID <<'_' <<name <<" n=" <<c_children <<" R=" <<R;
//  if(isTerminal) os <<" data: " <<D;
}
...
void UILE_Solver::query(CB_Node *n){
  ...
  if(verbose>0) LOG(0) <<"querying " <<n->ID <<'_' <<n->name;

  if(!n->isComplete){ //incomplete
...
//    n->parent->comp_n += c;
    ...
    if(n->isComplete){
      if(n->l<0.) n->l = n->costHeuristic();
      if(verbose>0) LOG(0) <<"computed " <<n->ID <<'_' <<n->name <<" -> complete with c=" <<n->c <<" l=" <<n->l;
      CHECK_GE(n->l, 0., "lower bound was not computed");
      if(n->isTerminal) terminals.append(n);
      else nonTerminals.append(n);
    }else{
      if(verbose>0) LOG(0) <<"computed " <<n->ID <<'_' <<n->name <<" -> still incomplete with c=" <<n->c;
    }
    ...
    while(p){
      p->comp_C += c;
      if(n->isComplete && n->isTerminal) p->comp_n += 1.;
      p=p->parent;
    }

    //backup closedness
    if(n->isComplete && n->isTerminal){
      n->isClosed=true;
      CB_Node* p=n->parent;
      while(p){
        if(p->getNumDecisions()<0 || (int)p->R<p->getNumDecisions()) break; //not all possible children expanded
...
//  n->isTerminal = n->isTerminal();
  if(n->isComplete && n->isTerminal){
    ...
    y_now=y;
    if(verbose>0) LOG(0) <<"sampled " <<n->ID <<'_' <<n->name <<" -> return " <<y;
...
void UILE_Solver::runTrivial(uint k, double maxEffortPerCompute){
  ...
  for(uint i=0;i<k;i++){
    ...
    if(n->isComplete){
      if(n->isTerminal) n=&root;
      else n=select_computeChild(n);
    }else{
      if(n->c > maxEffortPerCompute){
        if(verbose>0) LOG(0) <<"compute " <<n->ID <<'_' <<n->name <<" -> *** aborted with c=" <<n->c;
        n=&root;
      }
    }
  }
...
CB_Node* UILE_Solver::select_compParent_IE(){
  ...
  for(CB_Node* n:nonTerminals){
    uint ncompl=0;
    for(auto& ch: n->children) if(ch->isComplete) ncompl++;
    n->n_children = ncompl;
    nTotal += ncompl;
  }
...
CB_Node* UILE_Solver::select_compParent_Tree(){
  ...
  while(!n->isTerminal){
    ...
    if(!m->isComplete) break; //m itself (as compParent) needs to be complete
...
CB_Node* UILE_Solver::getCheapestIncompleteChild(CB_Node *r){
  CB_Node* m=0;
  for(auto& ch:r->children) if(!ch->isComplete && (!m || ch->c < m->c)) m=ch.get();
  return m;
}
...
CB_Node* UILE_Solver::select_computeChild(CB_Node *r){
  ...
  if(r->getNumDecisions()<0 || (int)r->R < r->getNumDecisions()){
    if(!j || j->c>get_novelThreshold(r)){ //not good enough -> create a new child
      r->children.append(r->getNewChild(r->R));
      j = r->children(-1).get();
      r->R++;
      if(verbose>0) LOG(0) <<"created new child ID:" <<j->ID <<" of type '" <<rai::niceTypeidName(typeid(*j)) <<"'";
    }
  }

  if(!j){ //all possible children are complete
    if(verbose>0) LOG(0) <<"all children of r=" <<r->name <<" complete -- removing it from nonTerminals -- selected none";
    nonTerminals.removeValue(r);
  }else{
    if(verbose>0) LOG(0) <<"chose j=" <<j->name <<" as cheepest compute child of r=" <<r->name;
  }
...
void UILE_Solver::report(){
  ...
  for(CB_Node* n:terminals){
    groundTruthMean += n->groundTruthMean();
    if(n->groundTruthMean()>groundTruthBest) groundTruthBest=n->groundTruthMean();
  }
