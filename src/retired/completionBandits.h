...
//  arr D;           //data at leafs
  ...
  CB_Node(CB_Node* _parent);

  virtual double costHeuristic(){ return 0.; }
  virtual double effortHeuristic(){ return 0.; }
  virtual double compute(){ HALT("need to overload"); }
  virtual double sample(){ HALT("need to overload"); }
  virtual double groundTruthMean(){ HALT("need to overload"); }

  virtual int getNumDecisions() = 0;
  virtual shared_ptr<CB_Node> getNewChild(uint i) = 0;
